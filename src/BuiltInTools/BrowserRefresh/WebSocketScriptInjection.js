  ...
  connection.onmessage = function (message) {
    if (message.data === 'Reload') {
      ...
      if (parsed.type == 'UpdateStaticFile') {
        if (parsed.updateScopedCss) {
          updateScopedCss();
        } else if (parsed.path && parsed.path.endsWith('.css')) {
          updateCssByPath(parsed.path);
        } else {
          console.debug(`File change detected to css file ${fileName}. Reloading page...`);
          location.reload();
          return;
        }
      }
